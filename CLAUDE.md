...
1. **Single-token (matvec) path** — `step_matvec_topk` / `encode_step_matvec` / `layer_pre_kv_in_pass` / `layer_post_kv_in_pass`. Used for **all of `--mode gen`** and for the generation phase of `--mode prompt`. Operates on `m=1` token; uses `matvec_q1_0` and the fused variant. The whole forward step (embed → 36× transformer layers → output_norm → LM head → topk_reduce) is encoded into **one** compute pass per step — the per-layer `kv_writeback_fused` kernel both folds in K's rms_norm + `*w_k_norm` + NEOX-RoPE before quantizing K/V to Q8_0, and writes both into the cache without breaking the pass (Q's rms_norm + RoPE remain separate dispatches because Q doesn't go through KV writeback). After the GPU step, the CPU reads back up to `TOPK_MAX = 32` candidates from `sample[0..2K]` and performs CPU-side sampling. Attention in this path uses the split-K kernel pair (see below).

2. **Batched-prefill (matmul) path** — `prefill_matmul_topk` / `layer_step_matmul`. Used by `Session::prefill` and the bin's `--mode prompt`. Quantizes activations to Q8_0 (`quantize_q8_0.wgsl`), then uses `dot4I8Packed`-based `matmul_q1_0_q8_0.wgsl` for the projections. The `kv_writeback_fused` kernel handles the K rms_norm + RoPE + Q8_0 quantize and the V Q8_0 quantize for all M tokens straight into the cache. Requires `pos_base == 0` (the matmul attention kernel assumes a fresh KV cache); for incremental prefill into an existing context, use `Session::prefill_one_at_a_time` (matvec-loop variant). Attention in this path uses the original single-pass `attention.wgsl` (`m_tokens=M`, `is_prefill=1`).
...
`begin_compute_pass` costs ~25us on RADV. The `_in_pass` family of helpers (`dispatch_rms_norm`, `dispatch_matvec_q1_0`, `dispatch_matvec_q1_0_fused`, `dispatch_silu_mul`, `dispatch_rope`, `dispatch_kv_writeback_fused`, `dispatch_topk_reduce`) all accept a caller-provided `&mut wgpu::ComputePass<'_>` so many dispatches share one pass. The matvec generation step is encoded as **a single** big pass — see `encode_step_matvec`. (Historical note: an earlier version split it in two around a `copy_buffer_to_buffer` for the K/V cache write; that copy was replaced by the kv_writeback kernel, and that kernel was later expanded into `kv_writeback_fused` which also folds in K's rms_norm + RoPE — eliminating the pass break and 2 dispatches/layer.) The plain wrappers (`rms_norm`, `silu_mul`, `matmul_q1_0`, `quantize_act`, etc.) open their own pass and are used in the batched-prefill path where pass-cost is amortized over batch size.
