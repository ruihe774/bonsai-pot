...
/// Patch the default value of a `[[function_constant(N)]]` declaration in an
/// MSL source string. wgpu's Metal passthrough path (like the Vulkan one) doesn't
/// expose `MTLFunctionConstantValues` to the application, so instead of binding
/// values at pipeline-creation time we rewrite the MSL declaration into a plain
/// constant-expression definition with the runtime value baked in.
///
/// Match: `constant uint <NAME> [[function_constant(<slot>)]];`
/// Replace: `constant uint <NAME> = <value>u;`
///
/// One occurrence per file (asserted). The shaders are generated by us so the
/// line shape is fixed; a unit test pins the expected pre/post strings.
#[cfg(target_vendor = "apple")]
#[allow(clippy::panic, reason = "shaders are written by us")]
fn msl_set_function_const_u32(src: &str, slot: u32, value: u32) -> String {
    let needle = format!("[[function_constant({slot})]];");
    let attr_pos = src
        .find(&needle)
        .unwrap_or_else(|| panic!("MSL function_constant({slot}) not found"));
    // Walk back from the attribute to the start of the line ("constant uint NAME ").
    let line_start = src[..attr_pos].rfind('\n').map_or(0, |i| i + 1);
    let prefix = &src[line_start..attr_pos];
    // Expect "constant uint NAME " (trailing space before the attribute).
    let stripped = prefix
        .strip_prefix("constant uint ")
        .and_then(|rest| rest.strip_suffix(' '))
        .unwrap_or_else(|| panic!("MSL function_constant line has unexpected shape: {prefix:?}"));
    let name = stripped;
    ...
    out.push_str(&src[attr_pos + needle.len()..]);
    // Sanity: only one occurrence per file.
    assert!(
        !out.contains(&needle),
        "more than one [[function_constant({slot})]] in MSL source"
    );
    out
}
...
impl Model {
    ...
    pub fn load_with_options(model_dir: &Path, opts: LoadOptions) -> Result<Self> {
        ...
        // SAFETY: every kernel ships either as a precompiled SPIR-V binary built
        // by `build.rs` from a hand-written GLSL source under `src/shaders/` (Vulkan
        // backend) or as a hand-ported MSL source compiled by Metal at load time
        // (Apple backend). For SPIR-V the three former WGSL text substitutions
        // (`{{SUBGROUP_SIZE}}`, `{{MAX_CHUNKS}}`, `{{N_EMBD_V4}}`) are Vulkan
        // specialization constants at SpecId 0, 1, 2; we patch their
        // `OpSpecConstant` defaults at load time. For MSL the SUBGROUP_SIZE
        // (slot 0) is baked as a literal 32 (Apple Silicon simdgroup width),
        // and MAX_CHUNKS / K_V4 (slots 1, 2) remain as Metal function constants;
        // we substitute them at load time via `msl_set_function_const_u32`.
        // Either way the result is fed through `create_shader_module_passthrough`,
        // bypassing naga.
        #[cfg(not(target_vendor = "apple"))]
        ...
        macro_rules! load_shader {
            ($name:expr, $spec_consts:expr, $wg:expr) => {{
                const SRC: &str = include_str!(concat!("shaders/", $name, ".metal"));
                let mut s: String = SRC.to_owned();
                for &(id, value) in $spec_consts as &[(u32, u32)] {
                    if id == SPEC_SUBGROUP_SIZE {
                        // Slot 0 (SUBGROUP_SIZE) is baked as a literal in the MSL.
                        continue;
                    }
...
mod tests {
    ...
    #[cfg(target_vendor = "apple")]
    #[test]
    fn msl_patcher_rewrites_function_constant() {
        ...
        assert!(s.contains("constant uint MAX_CHUNKS = 1234u;"));
        assert!(s.contains("constant uint K_V4 = 7u;"));
        assert!(!s.contains("function_constant(1)"));
        assert!(!s.contains("function_constant(2)"));
    }
