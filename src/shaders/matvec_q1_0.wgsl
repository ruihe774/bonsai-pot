...
// Layout is **transposed** vs. block-major (`[i_off * TILE_NB_Q8 + block_idx]`
// instead of `[block_idx * 8 + i_off]`) to dodge LDS bank conflicts: the
// matvec inner has WG_X=8 lanes-per-row reading the same `i_off` but
// different `block_idx` (= different `b_local`); block-major would stride
// those 8 reads by 32 u32s → all 8 lanes map to one bank → 8-way
// serialization. Transposed, the 8 lanes' addresses differ by 4 → 8 banks.
