...
The hot-path kernel `shaders/matvec_q1_0.wgsl` therefore needs **no multiplications inside the inner loop** — it accumulates `±x` per weight via `select(-xv, xv, bit_set)` and only multiplies by `d` once per block. `matvec_q1_0_fused_normed.wgsl` packs 2- or 3-range dispatches (QKV; gate+up) into one workgroup to amortize x-load cost, and additionally folds `rms_norm(x) * w_norm` over the activation so there's no `act.x_norm` round-trip.
