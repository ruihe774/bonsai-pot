...
- `src/shaders/*.wgsl` — one shader per kernel kind. The matvec/matmul shaders are the perf-critical ones; `topk_reduce.wgsl` is the sampler helper; `kv_writeback.wgsl` quantizes per-step K/V activations to Q8_0 directly into the KV cache; `probe_subgroup.wgsl` is the runtime probe used to bake `SG_SIZE` into the other shaders.
...
1. **Single-token (matvec) path** — `step_matvec_topk` / `encode_step_matvec` / `layer_pre_kv_in_pass` / `layer_post_kv_in_pass`. Used for **all of `--mode gen`** and for the generation phase of `--mode prompt`. Operates on `m=1` token; uses `matvec_q1_0` and the fused variant. The whole forward step (embed → 36× transformer layers → output_norm → LM head → topk_reduce) is encoded into **one** compute pass per step — the per-layer `kv_writeback` kernel replaced the old `copy_buffer_to_buffer` that used to break the pass. After the GPU step, the CPU reads back up to `TOPK_MAX = 32` candidates from `sample[0..2K]` and performs CPU-side sampling. Attention in this path uses the split-K kernel pair (see below).

2. **Batched-prefill (matmul) path** — `prefill_matmul_topk` / `layer_step_matmul`. Used by `Session::prefill` and the bin's `--mode prompt`. Quantizes activations to Q8_0 (`quantize_q8_0.wgsl`), then uses `dot4I8Packed`-based `matmul_q1_0_q8_0.wgsl` for the projections. After RoPE, the per-layer `kv_writeback` kernel quantizes K/V for all M tokens straight into the Q8_0 cache. Requires `pos_base == 0` (the matmul attention kernel assumes a fresh KV cache); for incremental prefill into an existing context, use `Session::prefill_one_at_a_time` (matvec-loop variant). Attention in this path uses the original single-pass `attention.wgsl` (`m_tokens=M`, `is_prefill=1`).
...
`rms_norm`, `attention`, `attention_split`, `attention_merge`, `quantize_q8_0`, and `kv_writeback` use `subgroupAdd` / `subgroupMax` with a runtime cross-subgroup merge when `num_subgroups > 1`. The cross-subgroup shared-memory arrays are sized at compile time for the **worst case**: `SG_PARTIAL_MAX = ceil(WG / SUBGROUP_MIN_SIZE)` slots, where `SUBGROUP_MIN_SIZE` is baked from `adapter.get_info().subgroup_min_size` at `Model::load` time via `{{SUBGROUP_MIN_SIZE}}` text replacement. The actual per-dispatch subgroup count is read from `@builtin(num_subgroups)` and individual subgroup indices from `@builtin(subgroup_id)` — no division by a compile-time constant.
...
`begin_compute_pass` costs ~25us on RADV. The `_in_pass` family of helpers (`dispatch_rms_norm`, `dispatch_matvec_q1_0`, `dispatch_matvec_q1_0_fused`, `dispatch_silu_mul`, `dispatch_rope`, `dispatch_kv_writeback`, `dispatch_topk_reduce`) all accept a caller-provided `&mut wgpu::ComputePass<'_>` so many dispatches share one pass. The matvec generation step is encoded as **a single** big pass — see `encode_step_matvec`. (Historical note: an earlier version split it in two around a `copy_buffer_to_buffer` for the K/V cache write; that copy was replaced by the `kv_writeback` kernel, which both quantizes K/V to Q8_0 and writes it into the cache — eliminating the pass break.) The plain wrappers (`rms_norm`, `silu_mul`, `matmul_q1_0`, `quantize_act`, etc.) open their own pass and are used in the batched-prefill path where pass-cost is amortized over batch size.
